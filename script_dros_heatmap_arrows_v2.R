...
df <- ggplot2df_uniq %>% 
  mutate(magn = abs(err_after_bbmerge) - abs(err_lsh_GTDB),
         direction = ifelse(abs(err_after_bbmerge) - abs(err_lsh_GTDB) > 0, "Up", "Down" ))%>%
  melt(id = c("sp", "dros_combo", "direction","magn"))
...
g1 <- ggplot(df, aes(x=as.factor(sp), y = value, color = variable,)) + 
  ...
  theme(legend.position = c(0.8,.18), legend.spacing.y = unit(0.05, 'cm'), legend.margin=margin(t = 0.1, unit='cm'),)+
  scale_color_manual(name="Absolute Error ...", values = c("#f4a582", "#0571b0"), labels = c( "Increased", "Reduced") )+
  #scale_color_gradient2(low="red",mid="grey60",high ="blue",midpoint = 0)+
...
g1 <- ggplot(df, aes(x=as.factor(sp), y = value, color = variable,)) + 
  #geom_point(size=1) + 
  geom_path(aes(color = magn, group = sp), arrow=arrow(angle = 10, length = unit(0.1, "inches"),  type = "open"))+
  theme_dark()+
  geom_hline(yintercept=0.0, color='grey', linetype="dashed", size = 0.4)+
  labs(y= "Change in relative error", x = "")+
  theme(axis.text.x = element_text(angle = 45, vjust = 1.0, hjust=1.0, size=5),)+
