...
df = read.csv('dros_distances.csv')
print (df)

colnames(df)

df_filt = read.csv('lsh_gtdb_results_dros_queries.csv')
#df_filt = read.csv('kraken_gtdb_results_dros_queries.csv')
df_filt
...
qplot(abs(df$err_lsh_GTDB)-abs(df$err_kraken_GTDB))+
  theme_bw()+
...
t.test(abs(df$err_lsh_GTDB)-abs(df$err_kraken_GTDB), alternative = c("two.sided"))

summary(abs(df$err_lsh_GTDB))
summary(abs(df$err_kraken_GTDB))
?t.test



#tile plot without percent filtered, compares before (after bbmerge) and after (after lsh)
...
#scale_y_continuous(labels=percent,name="Delta relative error after Kraken")+
#  scale_x_continuous(name=("Proportion filtered by Kraken"),labels=percent)+geom_hline(color="red",linetype=2,yintercept = 0)
#ggsave("Drosophila_tile_plot_kraken_vs_lsh_on_GTDB.pdf",width=9,height = 8)
ggsave("Drosophila_tile_plot_kraken_vs_lsh_on_BacArKraken.pdf",width=9,height = 8)
#ggsave("test.pdf",width=9,height = 8)
